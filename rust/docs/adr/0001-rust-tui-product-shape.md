...
- Persistent Luma-owned state lives under LumaNext (and tempfile overrides in tests). Explicit
  ...
  the boundary (for example Projects; Notes was retired), or use a narrowly scoped platform
  adapter for an opt-in system integration such as Keychain or system-proxy ownership. Those
  adapters must preserve their local safety, ownership, and rollback rules; modules do not
  acquire broader filesystem or system-write access.
