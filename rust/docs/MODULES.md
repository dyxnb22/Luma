...
| Proxy | `/proxy` / `/px` | Available — compact core/ownership status + active group overview; `/proxy group NAME` drills into nodes (on-demand Test Latency); mode, `/proxy status` and on-demand `/proxy check`, transactional local macOS HTTP/HTTPS/SOCKS proxy controls, safe Luma Profile import/list/use/delete/refresh, `/proxy sync` drafts, and one-step `/proxy apply` for `LumaNext/proxy.yaml`. Explicit standalone Mihomo controllers do not fall back to or mutate Clash Verge; switching from another system proxy is confirmed and restores that proxy on Disable. See [Proxy](./PROXY.md). | on |
