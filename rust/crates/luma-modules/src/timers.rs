...
use luma_application::{
    ...
    SearchSink, SpeechAccent, SpeechPort, TimerEntry, TimersRepository, WarmupContext,
    WorkbenchMeta,
};
use luma_domain::{
...
const DEFAULT_POMO_MINUTES: u32 = 25;
const MAX_COUNTDOWN_MINUTES: u32 = 24 * 60;
...
impl TimersModule {
    ...
    fn now_ms(&self) -> Result<i64, String> {
...
fn timer_title(entry: &TimerEntry, now_ms: i64) -> String {
...
fn timer_subtitle(entry: &TimerEntry) -> String {
...
fn primary_for(entry: &TimerEntry) -> (&'static str, &'static str) {
...
fn parse_minutes_token(token: &str) -> Option<u32> {
...
fn parse_countdown_spec(rest: &str) -> Option<(u32, String)> {
    let rest = rest.trim();
    if rest.is_empty() {
...
fn parse_stopwatch_name(rest: &str) -> String {
    let rest = rest.trim();
    let stripped = rest
        .strip_prefix("sw ")
...
fn timer_id_from_item(item: &SearchItem) -> Option<&str> {
...
fn payload_str(item: &SearchItem, key: &str) -> Option<String> {
    item.action_payload
        .as_ref()
...
fn payload_u64(item: &SearchItem, key: &str) -> Option<u64> {
    item.action_payload
        .as_ref()
        .and_then(|p| p.get(key))
        .and_then(|v| v.as_u64())
