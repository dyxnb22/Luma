...
| Proxy | `/proxy` / `/px` | Available — Clash-style keyboard settings home for System Proxy, explicit Rule/Global mode selection, current node, configuration, checks, and runtime; `/proxy group NAME` drills into a node-only list (on-demand Test Latency). Includes transactional local macOS HTTP/HTTPS/SOCKS controls, safe Luma Profile import/list/use/delete/refresh, `/proxy sync` drafts, and one-step `/proxy apply` for `LumaNext/proxy.yaml`. Global mode routes the `GLOBAL` selector through `PROXY`; explicit standalone controllers do not fall back to or mutate Clash Verge. See [Proxy](./PROXY.md). | on |
