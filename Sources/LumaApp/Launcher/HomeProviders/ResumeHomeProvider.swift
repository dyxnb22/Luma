...
struct ResumeHomeProvider: LauncherHomeProvider {
    ...
    init(enablementGate: HomeEnablementGate = HomeEnablementGate()) {
    ...
    func items() async -> [ResultItem] {
        ...
        if enablementGate.contains(.translate),
           !state.translateSource.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
        ...
        var snippetDraftRowShown = false
        if enablementGate.contains(.snippets),
           let data = state.snippetDraftJSON,
        ...
        var quicklinkDraftRowShown = false
        if enablementGate.contains(.quicklinks),
           let data = state.quicklinkDraftJSON,
        ...
        if enablementGate.contains(.todo),
           let todoText = state.todoCaptureText,
        ...
        if let moduleRaw = state.moduleRaw,
           ...
           ResumeHomeResumePolicy.allowsModuleResume(
               snippetDraftRowShown: snippetDraftRowShown,
               quicklinkDraftRowShown: quicklinkDraftRowShown
           ) {
            let moduleID = ModuleIdentifier(rawValue: moduleRaw)
            if enablementGate.contains(moduleID) {
        ...
        return Array(rows.prefix(HomeSuggestionPolicy.maxResumeRows))
    }
}
