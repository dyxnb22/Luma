...
- Accessibility permission is lazy: show the launcher banner only on AX-dependent surfaces — targeted queries for Snippets (`s`/`snip`), Window Layouts, or Menu Bar Search (`mb`); module detail for those modules; or after the user toggles Open Apps window controls (collapse/expand). Empty home and ordinary app search must not show the banner. Invoke the system prompt only from explicit user action (banner Grant or inline module permission row).
...
- Tab or ⌘K opens the action panel; Return runs the highlighted action (default primary). Command+Return runs `secondaryActions.first` when present.
