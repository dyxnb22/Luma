...
| SSH | `ssh` | Available — reads `~/.ssh/config` Host aliases (no wildcards); favorite/recent metadata in `ssh_meta.sqlite`; Enter connects in current terminal via OpenSSH; optional SFTP action; does not modify SSH config or read private keys | on |
