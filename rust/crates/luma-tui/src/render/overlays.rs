...
pub(super) fn render_overlay_doctor(
    frame: &mut Frame<'_>,
    area: Rect,
    state: &AppState,
    theme: &Theme,
    ...
    let full = state.doctor_diagnostic.as_ref().map_or_else(
        ...
        |d| format_doctor_panel(d, state.doctor_show_raw),
    );
...
pub(super) fn format_doctor_panel(diagnostic: &serde_json::Value, show_raw: bool) -> String {
    if show_raw {
        return serde_json::to_string_pretty(diagnostic).unwrap_or_else(|_| diagnostic.to_string());
    }
    let summary = format_doctor_summary(diagnostic);
    if summary.is_empty() {
...
pub(crate) fn format_doctor_summary(diagnostic: &serde_json::Value) -> String {
