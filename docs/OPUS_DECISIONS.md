...
- Accessibility permission is lazy: show launcher guidance only when enabled AX-dependent surfaces need it (Snippets, Window Layouts, Menu Bar Search, deferred Windows if enabled later, and Open Apps window expansion when AX is required); invoke the system prompt only from explicit user action.
