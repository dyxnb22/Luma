...
| Resume | `resume` / `rs` | Available — explicit work-entry contexts; `resume` / `resume list`; `resume <name>` restores safe entry (project/docs/editor/notes); `resume save\|capture\|edit\|set\|delete <name>`; SSH connect and recipe run require confirmation; data in `resume-contexts.json` under LumaNext | on |
